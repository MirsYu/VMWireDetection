...
{
    public class VC2000_Light
    {
        private const uint m_nMaxDurationTimeForPWM = 30000;

        CIOControllerSDK.MV_IO_SERIAL m_stSerial = new CIOControllerSDK.MV_IO_SERIAL();

        private String strTmpCom = "";

        private bool bComTen = false;
        ...
        public bool Open(int index)
        {
            ...
            {
                {
                    m_stSerial.strComName = "Com" + (index + 1);
                    if ((index + 1) >= 10)
                    {
                        bComTen = true;
                    ...
                    if (IntPtr.Zero == CIOControllerSDK.m_hDeviceHandleList[index]) // SelectedIndex也是从0开始
                    {
                        ...
                        {
                            CIOControllerSDK.m_hDeviceHandleList[index] = handle;
                        }
                    }
                    else
                    {
                    ...
                    nRet = CIOControllerSDK.MV_IO_Open_CS(CIOControllerSDK.m_hDeviceHandleList[index], ref m_stSerial);
                    if (CIOControllerSDK.MV_OK != nRet)
                    {
                        if (bComTen)
                        {
                            string str = ("The serial port " + strTmpCom + " is occupied.");
                        }
                        else
                        {
                            string str = ("The serial port " + m_stSerial.strComName + " is occupied.");
                        }

                        CIOControllerSDK.MV_IO_Close_CS(CIOControllerSDK.m_hDeviceHandleList[index]);
                        CIOControllerSDK.MV_IO_DestroyHandle_CS(CIOControllerSDK.m_hDeviceHandleList[index]);
                        CIOControllerSDK.m_hDeviceHandleList[index] = IntPtr.Zero;
                        return false;
                    }
                    else
                    {
                        if (bComTen)
                        {
                            string str = ("The serial port" + strTmpCom + "is available.");
                        }
                        else
                        {
                            string str = ("The serial port" + m_stSerial.strComName + "is available.");
                        }
                    }
                    return true;
                }
            ...
            {
                LogManager.WriteError("Sending the command failed. Parameters or operations exception occurs.");
                return false;
            }
        }
        ...
        public bool Close(int index)
        {
            ...
            if (IntPtr.Zero != CIOControllerSDK.m_hDeviceHandleList[index]) // SelectedIndex也是从0开始
            {
                if (bComTen)
                {
                    string str = (strTmpCom + "Handle deleted.");
                }
                else
                {
                    string str = (m_stSerial.strComName + "Handle deleted.");
                }
                CIOControllerSDK.MV_IO_Close_CS(CIOControllerSDK.m_hDeviceHandleList[index]);
                CIOControllerSDK.MV_IO_DestroyHandle_CS(CIOControllerSDK.m_hDeviceHandleList[index]);
                CIOControllerSDK.m_hDeviceHandleList[index] = IntPtr.Zero;
                ...
                if (bComTen)
                {
                    string str = (strTmpCom + " The serial port is closed.！");
                    LogManager.WriteInfo(str);
                }
                else
                {
                    string str = (m_stSerial.strComName + " The serial port is closed.");
                    LogManager.WriteInfo(str);
                }
            }
            else
            {
                string str = ("Error. The serial port is closed.");
                LogManager.WriteError(str);
            }
            return true;
        }

        public bool SetLight(int LightPort, int DurationTime, int LightValue, int LightState, int RisingEdge, int PortIndex = 0)
        {
            try
            {
                int nRet = CIOControllerSDK.MV_OK;
                CIOControllerSDK.MV_IO_LIGHT_PARAM stParam = new CIOControllerSDK.MV_IO_LIGHT_PARAM();
                stParam.nPortNumber = (Byte)(CIOControllerSDK.MV_IO_PORT_NUMBER)(LightPort);
                string strDurationTime = DurationTime.ToString();
                stParam.nLightValue = (ushort)LightValue;
                if (LightState == 1)
                {
                    stParam.nLightState = (UInt16)CIOControllerSDK.MV_IO_LIGHTSTATE.MV_IO_LIGHTSTATE_ON;
                }
                else if (LightState == 2)
                {
                    stParam.nLightState = (UInt16)CIOControllerSDK.MV_IO_LIGHTSTATE.MV_IO_LIGHTSTATE_OFF;
                }
                else
                {
                    stParam.nLightState = (UInt16)CIOControllerSDK.MV_IO_LIGHTSTATE.MV_IO_LIGHTSTATE_TRIGGER;
                }

                stParam.nLightEdge = (UInt16)(RisingEdge == 0 ? CIOControllerSDK.MV_IO_EDGE.MV_IO_EDGE_RISING : CIOControllerSDK.MV_IO_EDGE.MV_IO_EDGE_DOWN);
                if (strDurationTime == m_nMaxDurationTimeForPWM.ToString())
                {
                    stParam.nDurationTime = 30000;
                }
                else
                {
                ...
                nRet = CIOControllerSDK.MV_IO_SetLightParam_CS(CIOControllerSDK.m_hDeviceHandleList[PortIndex], ref stParam);
                if (CIOControllerSDK.MV_OK != nRet)
                {
                    return false;
                }
        ...
        public bool SetLightTrigger(int Port, int TriggerSource, int ComIndex = 0)
        {
            try
            {
                CIOControllerSDK.MV_IO_LIGHTSOURCE_PARAM stParam = new CIOControllerSDK.MV_IO_LIGHTSOURCE_PARAM();
                stParam.nLightPortNum = ((UInt16)(PORT_LIGHT_NUMBER)Port);
                stParam.nInputPortNum = (UInt16)(INPUT_PORT_NUMBER)TriggerSource;
                int nRet = CIOControllerSDK.MV_IO_SetLightTriggerSource_CS(CIOControllerSDK.m_hDeviceHandleList[ComIndex], ref stParam);
                if (CIOControllerSDK.MV_OK != nRet)
                {
                    return false;
                }
    ...
    public enum PORT_LIGHT_TRIGGERSOURCE
    {
        None = 0x0,
        Port1 = 0x1,
        Port2 = 0x2,
